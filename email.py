...
text_detials = \
        ...
        'To get more details, you can visit the following website: https://github.com/plana-planetarium/KToolBox_use/actions'
...
def get_text(status, next_status):
    text = \
            '[plana-planetarium/KToolBox_use] 下载kemono上的画师文件 (%s/3) %s successfully\n\n' % (str(status_num), str(status)) + \
...
def get_subject(status):
    subject = '[plana-planetarium/KToolBox_use] (%s/3) %s successfully' % (str(status_num), str(status))
    return subject
